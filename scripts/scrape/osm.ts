...
// Load extra city bboxes from JSON (generated by batch script)
function loadExtraCities(): Record<string, { south: number; west: number; north: number; east: number }> {
  try {
    const extraPath = path.join(__dirname, "cities-extra.json");
    if (fs.existsSync(extraPath)) {
      return JSON.parse(fs.readFileSync(extraPath, "utf8"));
    }
  } catch {}
  return {};
}
